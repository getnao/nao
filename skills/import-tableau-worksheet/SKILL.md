...
When the user requested a visualization, call `display_chart` with the query ID returned by the focused `execute_sql` call. Try to keep the original chart type, if impossible, choose the chart type from the analytical intent and the returned columns.
