...
export interface MattermostFormProps {
	...
	onSubmit: (values: { baseUrl: string; botToken: string }) => Promise<void>;
	onCancel: () => void;
	isPending: boolean;
}

export function MattermostForm({ hasProjectConfig, onSubmit, onCancel, isPending }: MattermostFormProps) {
	const form = useForm({
		defaultValues: { baseUrl: '', botToken: '' },
			...
			>
				...
				<div className='grid gap-3'>
					<p className='text-[11px] text-muted-foreground leading-relaxed'>
						<a
							href='https://docs.getnao.io/nao-agent/chat/mattermost'
							target='_blank'
							rel='noopener noreferrer'
							className='inline-flex items-center gap-1 underline hover:text-foreground'
						>
							See how to set up the Mattermost integration
