...
function CreateShareDialog({ open, onOpenChange, chatId }: ShareChatDialogProps) {
	...
	const shareMutation = useMutation(
		trpc.chatShare.create.mutationOptions({
			onSuccess: (data) => {
				queryClient.invalidateQueries({ queryKey: trpc.chatShare.findByChat.queryKey({ chatId }) });
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className='sm:max-w-md'>
				<DialogHeader>
					<DialogTitle>Share Chat</DialogTitle>
					<DialogDescription>
						Share a read-only link to this chat. Recipients can view but not edit.
					</DialogDescription>
				</DialogHeader>
...
function ManageShareDialog({
	...
	const deleteMutation = useMutation(
		trpc.chatShare.delete.mutationOptions({
			onSuccess: () => {
	...
	const updateAccessMutation = useMutation(
		trpc.chatShare.updateAccess.mutationOptions({
			onSuccess: () => {
				invalidateShareQueries();
				onOpenChange(false);
			},
	...
	const handleCopyLink = useCallback(() => {
		const url = `${window.location.origin}/chats/shared/${shareId}`;
		navigator.clipboard.writeText(url);
		setIsCopied(true);
		clearTimeout(timeoutRef.current);
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className='sm:max-w-md'>
				<DialogHeader>
					...
					<DialogDescription>
						This chat is currently shared. Recipients see the conversation in read-only mode.
					</DialogDescription>
				</DialogHeader>
				...
				<div className='flex flex-col gap-4'>
					<div className='flex items-center gap-3 rounded-lg border bg-muted/30 p-3'>
						{visibility === 'project' ? (
							...
							<>
								<div className='flex size-8 items-center justify-center rounded-full bg-blue-100 text-blue-600 dark:bg-blue-950 dark:text-blue-400'>
