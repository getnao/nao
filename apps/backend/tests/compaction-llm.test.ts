...
const mocks = vi.hoisted(() => ({
	generateText: vi.fn(),
	estimateMessage: vi.fn<(msg: ModelMessage) => number>().mockReturnValue(100),
}));
...
vi.mock('../src/services/token-counter', () => ({
	tokenCounter: { estimateMessage: mocks.estimateMessage },
	TokenCounter: vi.fn(),
}));
...
describe('selectMessagesInBudget', () => {
	beforeEach(() => {
		mocks.estimateMessage.mockReturnValue(100);
	});

	it('returns empty array for empty input', () => {
		expect(selectMessagesInBudget([], 1000)).toEqual([]);
	});
	...
	it('returns all messages when they all fit within the budget', () => {
		...
		expect(selectMessagesInBudget(messages, 10_000)).toEqual(messages);
	});
	...
	it('drops oldest messages when budget is exceeded', () => {
		...
		const result = selectMessagesInBudget(messages, 250);
	...
	it('excludes both tool and assistant messages when the pair exceeds budget', () => {
		...
		const result = selectMessagesInBudget(messages, 200);
	...
	it('stops selection when a tool message has no preceding assistant message', () => {
		...
		const result = selectMessagesInBudget(messages, 10_000);
...
describe('CompactionLLM', () => {
	...
	it('drops overflow messages from the start when budget is exceeded', async () => {
		...
		mocks.estimateMessage.mockReturnValue(100_000);
