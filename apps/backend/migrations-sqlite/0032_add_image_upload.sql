...
CREATE TABLE `__new_message_part` (
	...
	CONSTRAINT "text_required_if_type_is_text" CHECK(CASE WHEN "__new_message_part"."type" = 'text' THEN "__new_message_part"."text" IS NOT NULL ELSE TRUE END),
	CONSTRAINT "reasoning_text_required_if_type_is_reasoning" CHECK(CASE WHEN "__new_message_part"."type" = 'reasoning' THEN "__new_message_part"."reasoning_text" IS NOT NULL ELSE TRUE END),
	CONSTRAINT "tool_call_fields_required" CHECK(CASE WHEN "__new_message_part"."type" LIKE 'tool-%' THEN "__new_message_part"."tool_call_id" IS NOT NULL AND "__new_message_part"."tool_state" IS NOT NULL ELSE TRUE END),
	CONSTRAINT "file_fields_required" CHECK(CASE WHEN "__new_message_part"."type" = 'file' THEN "__new_message_part"."media_type" IS NOT NULL AND "__new_message_part"."image_id" IS NOT NULL ELSE TRUE END)
);
--> statement-breakpoint
...
CREATE INDEX `parts_message_id_order_idx` ON `message_part` (`message_id`,`order`);