...
UPDATE `project` SET `agent_settings` = json_set(`agent_settings`, '$.mapEnabled', json('true')) WHERE `agent_settings` IS NOT NULL AND json_extract(`agent_settings`, '$.experimental.displayMap') = 1 AND json_extract(`agent_settings`, '$.mapEnabled') IS NULL;--> statement-breakpoint
