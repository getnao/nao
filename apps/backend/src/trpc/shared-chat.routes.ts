...
export const sharedChatRoutes = {
	...
	create: projectProtectedProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			if (ownerId !== ctx.user.id) {
				throw new TRPCError({ code: 'FORBIDDEN', message: 'Only the chat owner can share this chat.' });
			}

			const existing = await sharedChatQueries.findByChat(input.chatId, ctx.user.id);
			if (existing) {
				await sharedChatQueries.deleteSharedChat(existing.id);
			}

			return sharedChatQueries.createSharedChat(
				{
	...
	findByChat: protectedProcedure.input(z.object({ chatId: z.string() })).query(async ({ input, ctx }) => {
		...
		if (ownerId !== ctx.user.id) {
			throw new TRPCError({ code: 'FORBIDDEN', message: 'Only the chat owner can manage sharing.' });
		}
	...
	updateAccess: projectProtectedProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			if (share.visibility !== 'specific') {
				throw new TRPCError({ code: 'BAD_REQUEST', message: 'Only specifically shared chats can be edited.' });
			}
