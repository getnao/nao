...
async function generateDynamicStoryCode(
	...
	const refreshModel = (await projectQueries.getAgentSettings(projectId))?.liveStoryRefresh;
	const provider = refreshModel?.provider ?? (await llmConfigQueries.getProjectDefaultModelProvider(projectId));
	if (!provider) {
		return null;
	}

	const modelId = refreshModel?.modelId ?? getDefaultModelId(provider);
