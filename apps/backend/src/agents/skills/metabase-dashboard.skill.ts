...
5. Group placements by question ID. Process each unique question once and reuse its nao query ID for repeated placements.
...
- For native SQL template tags, replace only exactly mapped Metabase optional clauses with nao filter blocks before execution. A direct \`column = {{tag}}\` clause can become \`{% filter tag %} AND column = {{ filters.tag.sql }} {% endfilter %}\`.
...
- Preserve each dashboard filter only when the story tool documents a matching filter type and its parameter mappings identify the exact target cards. Translate optional string equality and date-range template tags to nao filter blocks on those cards only.
...
- For an exact string filter whose native SQL clearly identifies \`table.column = {{tag}}\`, use a nao \`select\` or \`multi_select\` filter with that target table, column, and nao database ID so options come from distinct live values. Use \`search\` only when partial matching is intended and change the SQL operator to \`LIKE\`. If the option source or SQL wiring cannot be translated exactly, leave the filter inactive and report it instead of applying it globally.
