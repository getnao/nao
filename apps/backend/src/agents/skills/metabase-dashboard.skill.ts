...
5. Use each card's \`effectiveFilterIds\` from dashboard metadata; it contains every filter mapped in that tab for cards with at least one explicit mapping, and is empty for unwired cards. Group placements by question ID and this effective filter set, process each unique combination once, and reuse its nao query ID only for repeated placements with the same effective filters.
...
7. For each non-native or reusable question, batch up to five \`metabase://question/<id>\` URIs in one \`read_resource\` call. Call \`execute_question\` once only when compiled SQL is required. Read \`metabase://question/<id>/fields\` only when neither metadata nor the nao query result identifies the columns needed for an explicit visualization setting.
...
- Preserve virtual text cards as Markdown. Preserve repeated placements of a question.
...
- For native SQL template tags, replace mapped Metabase optional clauses with nao filter blocks before execution. For filters inherited from the tab, add the necessary optional predicate and any unambiguous joins using identifiers and relationships found in another source card's SQL or the nao database metadata; never invent them. A direct \`column = {{tag}}\` clause can become \`{% filter tag %} AND column = {{ filters.tag.sql }} {% endfilter %}\`.
...
- Preserve each dashboard filter only when the story tool documents a matching filter type. During Metabase dashboard migration only, apply the filters listed in each card's \`effectiveFilterIds\`; keep cards whose list is empty completely unfiltered. Use \`parameterMappings\` for the original Metabase targets and treat the remaining effective IDs as inherited tab filters. If an inherited filter cannot be translated from source SQL or database metadata without guessing, omit it from that card and report the limitation. Place each \`<filter>\` declaration only inside tabs containing a successfully translated target card.
