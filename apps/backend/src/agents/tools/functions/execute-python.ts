...
import type { Args, Ctx, ExternalFunction, Input, Output, Py, PyType, VirtualFile } from '../schema/execute-python';
...
function findAllFiles(dir: string, projectFolder: string): VirtualFile[] {
...
function loadProjectFiles(): VirtualFile[] {
...
const EXTERNAL_FUNCTION_MAP = new Map(EXTERNAL_FUNCTIONS.map((f) => [f.name, f]));
const EXTERNAL_FUNCTION_NAMES = EXTERNAL_FUNCTIONS.map((f) => f.name);

export const execute = async ({ code, inputs }: Input): Promise<Output> => {
