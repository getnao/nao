...
type WebDataset = {
	...
	lastPublishedProductCount?: number | null;
};
...
export function SystemPrompt({
	...
	return (
		<Block>
			...
			{webDatasets.length > 0 && <WebDatasetsBlock datasets={webDatasets} />}
	...
	return (
		<Block>
			...
			<Span>
				...
				nao's own DuckDB instead of a warehouse. It is always available, and it can do three things no warehouse
				can.
			</Span>
			<List>
				<ListItem>
					<Bold>Query a file by its path.</Bold> CSV, JSON, Parquet and Excel, read straight from{' '}
					<Bold>/home</Bold>, generated project data under <Bold>/datasets</Bold>, or the project folder — no
					loading step, and the file never enters your context. Use <Bold>read_csv</Bold>,{' '}
					<Bold>read_json</Bold>, <Bold>read_parquet</Bold> or <Bold>read_xlsx</Bold> and lastly{' '}
					<Bold>read_text</Bold> for text files (but use it only if you have to).{' '}
			...
			<Span>
				...
				SQL is rejected. It sees only the user's own saved files, generated project datasets, and the project
				folder. For questions a warehouse can answer on its own, keep using the warehouse — this is for files
				and for results you already have.
			</Span>
		</Block>
	);
}

function WebDatasetsBlock({ datasets }: { datasets: WebDataset[] }) {
	return (
		<Block>
			<Title level={2}>Web datasets</Title>
			<Span>
				These project datasets were generated by deterministic web robots. Query their published files under{' '}
				<Bold>/datasets</Bold> with the local database, and read each dataset README for its schema and
				provenance before answering.
			</Span>
			<List>
				{datasets.map((dataset) => (
					<ListItem key={dataset.slug}>
						<Bold>{dataset.name}</Bold>: <Bold>{`/datasets/${dataset.slug}/latest/README.md`}</Bold>
						{dataset.lastPublishedProductCount !== null && dataset.lastPublishedProductCount !== undefined
							? ` — ${dataset.lastPublishedProductCount} products`
							: ''}
						{dataset.lastSuccessfulRunAt ? ` — refreshed ${dataset.lastSuccessfulRunAt.toISOString()}` : ''}
					</ListItem>
				))}
			</List>
			<CodeBlock>
				{`SELECT product_key, name, sku, canonical_url\nFROM read_parquet('/datasets/${datasets[0]!.slug}/latest/products.parquet')\nLIMIT 20;`}
			</CodeBlock>
		</Block>
	);
}
