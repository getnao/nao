...
const CREATE_STORY_DESCRIPTION =
	'Create a new analytics story — a markdown document with embedded `<chart>` / `<table>` / `<grid>` / `<tabs>` ' +
	'blocks rendered by nao (think dashboard or report).\n\n' +
	'Default to a single flowing story. Use `<tabs>` only when requested or when clearly distinct sections are better separated than stacked; avoid tabs for short or single-topic stories. A tabbed story must start with `<tabs>` and contain only `<tab title="...">...</tab>` blocks, with no text outside a tab.\n\n' +
...
const UPDATE_STORY_DESCRIPTION =
	...
	'current value.\n\n' +
	'Preserve existing `<tabs>` structure unless the requested change makes tabs relevant or unnecessary; when using tabs, keep all content inside `<tab title="...">...</tab>` blocks.\n\n' +
	...
	registerMcpTool(server, ctx, {
		...
		inputSchema: {
			...
			content: z
				...
				.optional()
				.describe(
					'Full nao story markdown (with `<chart>`, `<table>`, `<grid>`, `<tabs>` blocks). Omit to start from a title-only stub.',
				),
			query_data: z
				.record(
					z.string(),
	...
	registerMcpTool(server, ctx, {
		...
		inputSchema: {
			...
			content: z
				...
				.optional()
				.describe(
					'Full markdown replacement (with `<chart>`, `<table>`, `<grid>`, `<tabs>` blocks). ' +
