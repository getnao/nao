...
UPDATE "project" SET "agent_settings" = jsonb_set("agent_settings", '{mapEnabled}', "agent_settings" #> '{experimental,displayMap}') WHERE "agent_settings" IS NOT NULL AND "agent_settings" #> '{experimental,displayMap}' IS NOT NULL AND NOT ("agent_settings" ? 'mapEnabled');--> statement-breakpoint
