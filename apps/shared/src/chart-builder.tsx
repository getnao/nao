...
export function buildChart(props: BuildChartProps) {
	...
	if (resolved.chartType === 'gauge') {
		return <GaugeChart {...resolved} />;
	}
...
function GaugeChart({
	...
	width = 540,
	height = 320,
}: ResolvedProps & { width?: number; height?: number }) {
	const minimum = gaugeSegments.reduce(
		(current, segment) => Math.min(current, segment.min),
		gaugeSegments[0]?.min ?? 0,
	);
	const maximum = gaugeSegments.reduce((current, segment) => Math.max(current, segment.max), minimum);
	const range = maximum - minimum;
	const metric = series[0];
	const value = toFiniteNumber(data[0]?.[metric?.data_key]);
	const formattedValue = value == null ? 'No numeric value' : formatPlainGaugeNumber(value);
	const displayedTitle = renderTitle ? title : undefined;
	const boundaries = [...new Set(gaugeSegments.flatMap((segment) => [segment.min, segment.max]))];
	const needleAngleDegrees = value == null || range <= 0 ? 180 : gaugeNeedleAngleDegrees(value, minimum, maximum);
	...
	return (
		<svg
			...
			role='img'
			aria-label={`${displayedTitle ? `${displayedTitle}: ` : ''}${formattedValue}`}
		...
		>
			...
			<path d={gaugeArcPath(180, 0)} fill='var(--muted, #e5e7eb)' />
			{range > 0 &&
				gaugeSegments.map((segment, index) => (
					<path
						key={index}
						d={gaugeArcPath(
							gaugeValueAngleDegrees(segment.min, minimum, maximum),
							gaugeValueAngleDegrees(segment.max, minimum, maximum),
						)}
						fill={segment.color}
					/>
				))}
			{range > 0 &&
				boundaries.map((boundary) => {
					const angleDegrees = gaugeValueAngleDegrees(boundary, minimum, maximum);
					const position = gaugePoint(GAUGE_CENTER_X, GAUGE_CENTER_Y, GAUGE_OUTER_RADIUS + 14, angleDegrees);
					return (
						<text
							key={`boundary-${boundary}`}
							x={position.x}
							y={position.y}
							textAnchor={gaugeLabelAnchor(angleDegrees, 14)}
							dominantBaseline='middle'
							fontSize={11}
							fontWeight='700'
							...
							paintOrder='stroke'
						>
							{formatPlainGaugeNumber(boundary)}
						</text>
					);
				})}
			{range > 0 &&
				gaugeSegments.map((segment, index) => {
					...
					const angleDegrees = gaugeValueAngleDegrees((segment.min + segment.max) / 2, minimum, maximum);
					const position = gaugePoint(GAUGE_CENTER_X, GAUGE_CENTER_Y, GAUGE_OUTER_RADIUS + 34, angleDegrees);
					return (
						<text
							key={index}
							x={position.x}
							y={position.y}
							textAnchor={gaugeLabelAnchor(angleDegrees, 34)}
							dominantBaseline='middle'
							fontSize={12}
							fontWeight='700'
							fill='var(--foreground, #111827)'
			...
			{value != null && range > 0 && (
				<>
					<polygon
						points={gaugeTrianglePoints(needleAngleDegrees, GAUGE_NEEDLE_OUTLINE_RADIUS)}
						fill='var(--background, #ffffff)'
					/>
					<polygon
						points={gaugeTrianglePoints(needleAngleDegrees, GAUGE_NEEDLE_RADIUS)}
						fill='var(--foreground, #111827)'
					/>
				</>
			)}
			<text
				...
				textAnchor='middle'
				dominantBaseline='middle'
				fontSize={value == null ? 16 : 32}
				fontWeight={value == null ? '400' : '600'}
				fill={value == null ? 'var(--muted-foreground, #6b7280)' : 'var(--foreground, #111827)'}
			>
				{formattedValue}
			</text>
		</svg>
...
const GAUGE_WIDTH = 540;
const GAUGE_HEIGHT = 320;
const GAUGE_CENTER_X = GAUGE_WIDTH / 2;
const GAUGE_CENTER_Y = 215;
const GAUGE_OUTER_RADIUS = 140;
const GAUGE_INNER_RADIUS = 106;
const GAUGE_NEEDLE_RADIUS = 10;
const GAUGE_NEEDLE_OUTLINE_RADIUS = GAUGE_NEEDLE_RADIUS * 1.4;
const GAUGE_NEEDLE_CENTER_RADIUS = GAUGE_INNER_RADIUS + GAUGE_NEEDLE_RADIUS * Math.tan(Math.PI / 6);

function gaugeValueAngleDegrees(value: number, minimum: number, maximum: number): number {
	return 180 - ((value - minimum) / (maximum - minimum)) * 180;
}

function gaugeNeedleAngleDegrees(value: number, minimum: number, maximum: number): number {
	return gaugeValueAngleDegrees(Math.min(maximum, Math.max(minimum, value)), minimum, maximum);
}

function formatPlainGaugeNumber(value: number): string {
	return value.toLocaleString('en-US', { maximumFractionDigits: 20 });
}

function gaugeLabelAnchor(angleDegrees: number, labelOffset: number): 'start' | 'middle' | 'end' {
	const centeredThresholdDegrees =
		(Math.acos(GAUGE_OUTER_RADIUS / (GAUGE_OUTER_RADIUS + labelOffset)) * 180) / Math.PI;
	if (Math.abs(angleDegrees - 90) <= centeredThresholdDegrees) {
		return 'middle';
	}
	return angleDegrees > 90 ? 'end' : 'start';
}

function gaugeTrianglePoints(angleDegrees: number, radius: number): string {
	const radians = (angleDegrees * Math.PI) / 180;
	const radial = { x: Math.cos(radians), y: -Math.sin(radians) };
	const tangent = { x: Math.sin(radians), y: Math.cos(radians) };
	const center = gaugePoint(GAUGE_CENTER_X, GAUGE_CENTER_Y, GAUGE_NEEDLE_CENTER_RADIUS, angleDegrees);
	const baseOffset = radius / 2;
	const halfBase = (radius * Math.sqrt(3)) / 2;
	return [
		`${center.x + radial.x * radius},${center.y + radial.y * radius}`,
		`${center.x - radial.x * baseOffset + tangent.x * halfBase},${center.y - radial.y * baseOffset + tangent.y * halfBase}`,
		`${center.x - radial.x * baseOffset - tangent.x * halfBase},${center.y - radial.y * baseOffset - tangent.y * halfBase}`,
	].join(' ');
}

function gaugePoint(centerX: number, centerY: number, radius: number, angleDegrees: number) {
	const radians = (angleDegrees * Math.PI) / 180;
	return {
...
function gaugeArcPath(startAngleDegrees: number, endAngleDegrees: number): string {
	const outerStart = gaugePoint(GAUGE_CENTER_X, GAUGE_CENTER_Y, GAUGE_OUTER_RADIUS, startAngleDegrees);
	const outerEnd = gaugePoint(GAUGE_CENTER_X, GAUGE_CENTER_Y, GAUGE_OUTER_RADIUS, endAngleDegrees);
	const innerEnd = gaugePoint(GAUGE_CENTER_X, GAUGE_CENTER_Y, GAUGE_INNER_RADIUS, endAngleDegrees);
	const innerStart = gaugePoint(GAUGE_CENTER_X, GAUGE_CENTER_Y, GAUGE_INNER_RADIUS, startAngleDegrees);
	return [
		`M ${outerStart.x} ${outerStart.y}`,
		`A ${GAUGE_OUTER_RADIUS} ${GAUGE_OUTER_RADIUS} 0 0 1 ${outerEnd.x} ${outerEnd.y}`,
		`L ${innerEnd.x} ${innerEnd.y}`,
		`A ${GAUGE_INNER_RADIUS} ${GAUGE_INNER_RADIUS} 0 0 0 ${innerStart.x} ${innerStart.y}`,
		'Z',
	].join(' ');
}
