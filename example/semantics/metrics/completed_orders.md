...
**Type:** simple  
**Description:** Number of orders whose status is completed.  
...
Over time:
```json
{"metrics": ["completed_orders"], "group_by": ["metric_time__month"], "order_by": ["metric_time__month"]}
```

Broken down, filtered and ranked (see `../README.md` for every parameter):
```json
{"metrics": ["completed_orders"], "group_by": ["customer__first_name"], "where": ["{{ Dimension('customer__first_name') }} IS NOT NULL"], "order_by": ["-completed_orders"], "limit": 10, "start_time": "2024-01-01", "end_time": "2024-12-31"}
```
