...
**Type:** simple  
**Description:** Amount covered by coupons.  
...
Over time:
```json
{"metrics": ["coupon_revenue"], "group_by": ["metric_time__month"], "order_by": ["metric_time__month"]}
```

Broken down, filtered and ranked (see `../README.md` for every parameter):
```json
{"metrics": ["coupon_revenue"], "group_by": ["customer__first_name"], "where": ["{{ Dimension('customer__first_name') }} IS NOT NULL"], "order_by": ["-coupon_revenue"], "limit": 10, "start_time": "2024-01-01", "end_time": "2024-12-31"}
```
